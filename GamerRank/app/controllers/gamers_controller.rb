class GamersController < ApplicationController
  ...
  def show
    ...
    scores = Gamer.compute_scores(@reviews)
    Gamer.compute_weighted_score(scores)
  ...
  private
    ...
    def compile_reviews(gamer_id)
      @reviews = Review.where(target_id: gamer_id)
    end

    def update_scores(scores={}, gamer)
      gamer.skill_rating = scores[:skill]
      gamer.team_rating = scores[:team]
      gamer.communication_rating = scores[:comm]
      gamer.play_again_rating = scores[:play]
      gamer.total_computed_score = scores[:weighted]
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def gamer_params
      params.require(:gamer).permit(:username, :xbox_tag, :psn_tag, :steam_tag)
    end
end
